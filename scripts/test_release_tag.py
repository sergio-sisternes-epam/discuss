...
class ReleaseTagTests(unittest.TestCase):
    ...
    def test_remote_annotated_tag_survives_checkout_ref_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.git(source, "tag", "-a", "v0.3.8", "-m", "Discuss v0.3.8")
            self.git(source, "push", "origin", "refs/tags/v0.3.8")
            ...
            self.git(
                ...
                "--no-tags",
                "origin",
                f"{commit}:refs/tags/v0.3.8",
            )
            self.assertEqual(
                self.git(checkout, "cat-file", "-t", "refs/tags/v0.3.8"),
                "commit",
            )

            verified = release_tag.verify_remote_tag("v0.3.8", checkout)

            self.assertEqual(verified.tag_ref, "refs/release-tags/v0.3.8")
    ...
    def test_lightweight_remote_tag_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                ...
                release_tag.verify_remote_tag("v0.3.8", checkout)
    ...
    def test_tag_not_on_current_main_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.git(source, "push", "origin", "main", "refs/tags/v0.3.8")
